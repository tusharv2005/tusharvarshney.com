---
title: Shimmering Text
description: Smooth, light-sweeping shimmer animation for text.
image: https://assets.tusharvarshney.com/images/blog/shimmering-text.webp
category: components
createdAt: 2025-11-02
updatedAt: 2026-02-20
---

<ComponentPreview
  name="shimmering-text-demo-01"
  openInV0Url="https://tusharvarshney.com/r/shimmering-text.json"
  remountOnThemeChange
/>

<DocSponsors />

## Installation

<CodeTabs>

<TabsListInstallType />

<TabsContent value="cli">

```bash
npx shadcn@latest add @tusharvarshney/shimmering-text
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies</Step>

```bash
npm install motion clsx tailwind-merge
```

<Step>Add a cn helper</Step>

<ComponentSource
  src="src/lib/utils.ts"
  title="lib/utils.ts"
  collapsible="false"
/>

<Step>Copy and paste the following code into your project</Step>

<ComponentSource
  name="shimmering-text"
  title="components/shimmering-text.tsx"
/>

<Step>Update the import paths to match your project setup</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { ShimmeringText } from "@/components/shimmering-text"
```

```tsx
<ShimmeringText text="slide to unlock" />
```

## API Reference

### ShimmeringText

<AutoTypeTable
  path="src/features/doc/content/props.ts"
  name="ShimmeringTextProps"
/>

## Examples

### Custom Color

<ComponentPreview name="shimmering-text-demo-02" remountOnThemeChange />

CSS Variables explanation:

- `--color`: Base text color (the default/resting state of characters)
- `--shimmering-color`: Peak highlight color (the bright color characters transition to during the shimmer effect)
